import 'package:face_recog_app/missing_list/missing_person_model.dart';
import 'missing_person_model.dart';

class MissingPersonController {
  late List<MissingPerson> lista;

  MissingPersonController() {
    lista = [
      MissingPerson(
          id: 01,
          name: 'Joao Silva',
          birthday: '01/01/2001',
          birthplace: 'Brasilia, Distrito Federal, Brasil',
          place: 'São Paulo, SP, Brasil',
          details:
              'Pessoa desaparecida no dia 01/06/2022 ao sair de uma festa no bairro X na hora Y',
          userId: 02),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03),
      MissingPerson(
          id: 02,
          name: 'Joao Gado',
          birthday: '01/01/2002',
          birthplace: 'Goiania, Goias, Brasil',
          place: 'Rio de Janeiro, RJ, Brasil',
          details:
              'Pessoa desaparecida no dia 02/06/2022 ao sair de uma festa no bairro Z na hora Y',
          userId: 03)
    ];
  }
}
